...
test('package keeps v1.16.2+ combat-feel safeguards',()=>ok(/^1\.(?:1[7-9]|[2-9]\d)\.|^1\.16\.[2-9]/.test(pkg.version),`version=${pkg.version}`));
...
test('player sprites remain native scale and torso hit circles are explicit',()=>{
  ...
  const playerBlock=game.slice(game.indexOf('createPlayer()'),game.indexOf('createInput()'));
  ok(!/displayWidth\s*=|displayHeight\s*=|setDisplaySize\s*\(/.test(playerBlock),'player display size deformation API found');
...
test('old spear-like rush warning is gone',()=>{ok(game.includes("create('rushLine', 128, 24"),'rushLine missing');ok(!game.includes("create('rushWarning'"),'old rushWarning texture remains');ok(!game.includes("texture:'rushWarning'"),'old rushWarning network ref remains');ok(game.includes("texture:'rushLine'"),'rushLine network ref missing');});
