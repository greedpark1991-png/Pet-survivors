...
OUT=ROOT/'tests'/'screens-v162'
OUT.mkdir(exist_ok=True)

# deterministic visual-only park crop mirroring v1.16.2 createWorld composition
W,H=960,540
...
if samples:
    d.polygon(polygon_h(42),fill=(118,106,87)); d.polygon(polygon_h(35),fill=(143,125,101)); d.polygon(polygon_h(30),fill=(154,136,111))
    for i,p in enumerate(samples[1:-1:3],1):
        x,y,_=p; d.rectangle((x-10,y-15+(i%4)*9,x+10,y-13+(i%4)*9),fill=(185,161,126))
    for i,p in enumerate(samples[2:-2:4],1):
        x,y,_=p; d.rectangle((x-2+((i%3)-1)*14,y-7+(i%4)*5,x+3+((i%3)-1)*14,y-5+(i%4)*5),fill=(124,104,79))
...
if samplesv:
    d.polygon(polygon_v(41),fill=(118,106,87)); d.polygon(polygon_v(35),fill=(143,125,101)); d.polygon(polygon_v(30),fill=(154,136,111))
    for i,p in enumerate(samplesv[1:-1:3],1):
        x,y,_=p; d.rectangle((x-15+(i%4)*9,y-10,x-13+(i%4)*9,y+10),fill=(185,161,126))
    for i,p in enumerate(samplesv[2:-2:4],1):
        x,y,_=p; d.rectangle((x-7+(i%4)*5,y-2+((i%3)-1)*14,x-5+(i%4)*5,y+3+((i%3)-1)*14),fill=(124,104,79))
...
with sync_playwright() as p:
    ...
    for vw,vh in [(1366,768),(1920,1080),(2560,1440)]:
        ...
        if vw==1920:
            # Choice
            page.evaluate('''() => {document.querySelector('#game-shell').classList.remove('game-running');document.querySelector('#levelup-screen').classList.add('show');document.querySelector('#levelup-choices').innerHTML='<button class="choice-card" data-type="offense"><div class="choice-top"><span class="icon">⚔</span><span class="choice-tag">기본 강화</span></div><b>공격력 증가</b><p class="choice-summary">모든 공격 피해 +7%</p><p class="choice-detail">모든 공격 피해량이 7% 증가한다.</p><span class="level">현재 Lv.3 → Lv.4</span></button><button class="choice-card" data-type="survival"><div class="choice-top"><span class="icon">♥</span><span class="choice-tag">기본 강화</span></div><b>최대 체력 회복</b><p class="choice-summary">최대 HP +14 · 즉시 회복</p><p class="choice-detail">최대 HP가 증가하고 체력을 회복한다.</p><span class="level">현재 Lv.1 → Lv.2</span></button><button class="choice-card exclusive-choice" data-type="exclusive"><div class="choice-top"><span class="icon">🐾</span><span class="choice-tag">캐릭터 전용</span></div><b>목청 폭발</b><p class="choice-summary">소리탄 충격파 확률 추가</p><p class="choice-detail">소리탄 적중 시 일정 확률로 작은 충격파가 터진다.</p><span class="level">NEW</span></button>'; }''')
            page.wait_for_timeout(240); page.screenshot(path=str(OUT/'choice-1920x1080.png'),full_page=True)
            # Pause
            page.evaluate('''() => {document.querySelector('#levelup-screen').classList.remove('show');document.querySelector('#pause-screen').classList.add('show');document.querySelector('#pause-stat-grid').innerHTML='<div><small>공격력</small><b>38.4</b></div><div><small>이동속도</small><b>246</b></div><div><small>공격속도</small><b>1.08/s</b></div><div><small>HP</small><b>73 / 100</b></div>';document.querySelector('#pause-basic-upgrades').innerHTML='<span>공격력 ×4</span><span>공속 ×3</span>';document.querySelector('#pause-special-list').innerHTML='<div>✚ 개이득 <b>Lv.3</b></div>';document.querySelector('#pause-major-list').innerHTML='<div>● 똥강아지 <b>Lv.2</b></div>';document.querySelector('#pause-skill-list').innerHTML='<div>✦ 마법미사일 <b>Lv.2</b></div>'; }''')
