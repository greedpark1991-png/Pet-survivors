...
test('package keeps v1.16.1+ visual polish foundations',()=>ok(pkg.version==='1.17.0'||pkg.version==='1.16.2'||pkg.version==='1.16.1',`version=${pkg.version}`));
...
test('player sprites use uniform native scale and current torso circles',()=>{
  ...
  ok(!/displayWidth\s*=|displayHeight\s*=|setDisplaySize\s*\(/.test(playerBlock),'display size APIs must not deform player sprites');
